...
制定日期：2026-09-23。状态：**Windows 元数据候选已实现并通过完整本地检查；等待刷新连接后的 A 版基线，尚未部署候选，原生端尚未修复。** 既有请求 ID、版本和日志见[排查记录](client-troubleshooting.md)。本方案独立于交给 5.6 sol 的 B 版视觉、分页和提交摘要改造。
...
当前待用户在浏览器刷新原连接后，用同一模型分别新建浏览器和 Windows 对话复测 A 版。收到该结果后再部署候选 B；B 有效时继续做 A 回退对照。未取得原生端结果前不能记为修复完成。
