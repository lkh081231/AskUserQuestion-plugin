...
截至 2026-09-22，阶段 0 已完成；阶段 2 的本地功能与边界测试已完成。Secure MCP Tunnel 已在真实 ChatGPT 中完成工具发现、四题型 UI 和答案回传验证，但阶段 1 仍缺少停止等待行为统计；阶段 3 已完成 skill 和包结构但缺少真实连接 ID 与完整安装；阶段 4 已提供并启用 user systemd 单元，Tunnel 服务等待 Runtime API Key credential，公网部署、法律/支持 URL 和公开审核仍受外部条件阻塞。实际证据见[验收记录](docs/acceptance.md)。
...
| 7 | 重复点击、发送成功、发送失败或接口不可用 | 一次点击流程只发起一次发送；成功后禁用；失败保留输入并可手动重试；接口不可用时可选中并手动复制 Q/A，不依赖 `navigator.clipboard` |
