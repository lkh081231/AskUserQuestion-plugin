...
| 宿主管理的弹窗 | ChatGPT 可选扩展 `window.openai.requestModal({ params, template })`；省略 `template` 使用当前模板 | 用户提出的候选方案。须检测方法是否存在，并验证题目初始化、提交结果和关闭后的摘要；不是标准 MCP Apps 的输入框绑定接口 |
...
| 宿主管理的弹窗 | 可用同一分页表单集中完成回答；官方提供 `requestModal` 入口 | 用户已询问此方案；当前连接支持情况、弹窗参数读取、关闭及原卡片摘要同步均未实测，不能仅凭方法名加入自动弹窗 |
...
5. 弹窗候选：先检测 `window.openai.requestModal`，记录同模板/指定模板的题目数据、提交路径、关闭结果和原卡片表现；不得由本地模拟推断跨视图状态同步。
6. 确认宿主是否真的存在额外支持的输入框绑定接口；若无证据，保持该验收项未完成。
