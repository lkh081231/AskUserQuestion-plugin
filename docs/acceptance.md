...
用户已确认预览提交 `2fe1f83`。B 版视觉、与输入框绑定、多题分页、Enter 操作和统一问答摘要已写入 [计划](../plan.md)及 [TODO](../TODO.md)，正式实现与真实宿主验收尚未执行。

| 新验收项 | 当前状态 |
| --- | --- |
| 18：选择行与键盘 | 独立 HTML 预览检查通过，正式 UI 待实现 |
| 19：多题分页和草稿 | 独立 HTML 预览检查通过，正式 UI 待实现 |
| 20：成功后统一摘要 | 独立 HTML 预览检查通过，真实 `ui/message` 状态待实现/验证 |
| 21：绑定宿主输入框 | 仅独立页面可实现，尚未核验真实宿主支持 |
...
2026-09-22 至 2026-09-23，用户报告浏览器正常、手机/iPad 原生端在调用时返回组织上下文 UNAUTHORIZED，Windows 原生端静默失败。同一账号已确认，具体工作区、客户端版本和失败请求时间仍待补充。

本地服务、Tunnel readiness、随机端口 doctor 及 Tunnel 元数据读取通过；这些检查不能替代原生客户端验收。手机/iPad 优先指向入口鉴权，Windows 根因尚未定位；**没有修复或跨端通过结论**。证据、假设和复测步骤见[客户端排查](client-troubleshooting.md)。
